                ...
                <?php if(!empty($getData)): ?>
                  <?php foreach($getData as $user): ?>
                    <tr>
                      ...
                      <td><a href="<?= site_url('admin/Users/UpdateUsers/'.$user['id']); ?>"><?= $user['mobile']; ?></a></td>
                      <td><?= $user['email_id']; ?></td>
                      <td><?= !empty($user['add_date']) ? date('d-m-Y', strtotime($user['add_date'])) : 'N/A'; ?></td>
                      <td class="text-center"><strong><?= $user['total_orders'] ?? 0; ?></strong></td>
                      <td class="text-center"><strong>₹<?= number_format($user['total_amount_spent'] ?? 0, 2); ?></strong></td>
                      <td>
                        <?php if(!empty($user['addresses'])): ?>
                          <?php foreach(array_slice($user['addresses'], 0, 2) as $addr): ?>
                            <div><i class="fa fa-map-marker text-danger"></i> <?= $addr['address']; ?>, <?= $addr['city']; ?></div>
                          <?php endforeach; ?>
                      ...
                      <td><a href="<?= site_url('admin/Users/UpdateUsers/'.$user['id']); ?>"><button class="btn btn-sm btn-primary">Edit</button></a></td>
                      <td><a href="<?= base_url('admin/Users/delete_user/'.$user['id']); ?>" onclick="return confirm('Are you sure you want to delete this user?');"><button class="btn btn-sm btn-danger">Delete</button></a></td>
                    </tr>
                  <?php endforeach; ?>
                <?php else: ?>
