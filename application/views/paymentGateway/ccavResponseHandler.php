    ...
    if ($key === 'order_id') {
        $order_number = $val;
          ...
          $get_oder  = $this->db->get_where('order_master2',array('order_number'=>$order_number))->row_array();
           ...
           $vendordata = $this->db->get_where('purchase_master2',array('order_master_id'=>$order_id))->row_array();
   ...
   //  $this->db->select('product_name,shop_id,sku_code,product_code,main_image');
    ...
    $this->db->insert('logs', $data);
         ...
         $this->db->select('mobile,name,email');
            $staff = $this->db->get_where('staff_master', array('id' => $vendordata['vendor_master_id']))->row_array();
      ...
      if($order_status==="Success"):
            ...
            $admin_message = "Dukekart has received an order on Product name: " . $vendordata['product_name'] . ", Seller Mobile no: ". $staff['mobile'] .".  Regards, Dukekart Real Time Private Limited , www.dukekart.in";
            ...
            // sendSMS('9198643301',$admin_message,'1007850092609093226');
                ...
                $address_data = $this->db->get_where('order_address_master2',array('order_master_id'=>$order_id))->row_array();
                 ...
                 foreach ($get_purchase as $purchaseValue) {
                     $this->db->insert('purchase_master',array('vendor_master_id'=>$purchaseValue['vendor_master_id'],'shop_id'=>$purchaseValue['shop_id'],'product_name'=>$purchaseValue['product_name'],'order_master_id'=>$last_id,'price'=>$purchaseValue['price'],'final_price'=>$purchaseValue['final_price'],'product_master_id' => $purchaseValue['product_master_id'],'quantity' => $purchaseValue['quantity'],'size' => $purchaseValue['size'],'color' => $purchaseValue['color'],'modify_date' => time(),'add_date' => time()));
                 }    
