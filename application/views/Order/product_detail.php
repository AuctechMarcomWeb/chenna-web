...
<!--Shipping Address  -->  
 ...
 <?php if($adminData['Type']=='1') { ?>
                ...
                <form class="form-horizontal"  action="<?php echo site_url('admin/Order/UpdateOrderStatus').'/'.$this->uri->segment(4); ?>" method="POST" enctype="multipart/form-data">
                ...
                <?php  $status = $getData['status'];?>
                 ...
                 <div class="form-group">
                  ...
                  <div class="col-xs-12 col-sm-10">
                   <?php  if($status =='3') { ?>
                    <input type="text" class="form-control"  required value="<?php echo ucwords('Order Delivered')?>" Disabled>
                    <?php } elseif($status=='4') { ?>
                    <input type="text" class="form-control"  required value="<?php echo ucwords('Order cancel by customer')?>" Disabled>
                    <?php } elseif($status=='6') { ?>
                    <input type="text" class="form-control"  required value="<?php echo ucwords('Order reject by seller')?>" Disabled>
                    <?php } elseif($status=='7') { ?>
                    <input type="text" class="form-control"  required value="<?php echo ucwords('Return Request')?>" Disabled>
                    <?php } elseif($status=='8') { ?>
                    <input type="text" class="form-control"  required value="<?php echo ucwords('Return Completed')?>" Disabled>
                    <?php } else {   ?>
                      <select name="StatusUpdate" style="color:black"  class="form-control">
                         <option value="1" <?php echo ($status =='1')?'Selected':'' ?>> Order waiting for seller approval</option>
                         <option value="5" <?php echo ($status =='5')?'Selected':'' ?>> Order confirmed by seller</option>
                         <option value="6" <?php echo ($status =='6')?'Selected':'' ?>> Order reject by seller</option>
                         <option value="2" <?php echo ($status =='2')?'Selected':'' ?>> Order shipped</option>
                         <option value="3" <?php echo ($status =='3')?'Selected':'' ?>> Order Delivered</option>
              ...
              <?php if($status!='3' && $status!='4' && $status!='6' && $status!='7' && $status!='8') { ?>
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-10 col-xs-12">
                    <textarea rows="5" cols ="3" class="form-control" required  name="remark"></textarea>
                  </div>
                </div>
                     ...
                     <div class="col-sm-12" style="overflow: auto;">
                      <table id="example1" class="table table-bordered table-striped">
                        ...
                        <tr>
                          ...
                          <td>
                             <?php 
                              if( $value['order_status']==1){ echo '<b style="color:#ff6c00">Order&nbsp;waiting&nbsp;for&nbsp;seller approval</b>';}
                              if($value['order_status']==4){ echo '<b style="color:#f00">Order&nbsp;cancel&nbsp;by customer</b>';}
                              if($value['order_status']==5){ echo '<b style="color:#ff6c00">Order&nbsp;confirmed&nbsp; by seller</b>';}
                              if($value['order_status']==6){ echo '<b style="color:green">Order&nbsp;reject&nbsp;by seller</b>';}
                              if($value['order_status']==3){ echo '<b style="color:green">Order&nbsp;Delivered</b>';}
                              if($value['order_status']==2){ echo '<b style="color:#ff6c00">Order&nbsp;shipped</b>';}
                              if($value['order_status']==7){ echo '<b style="color:green">Pickup Sheduled</b>';}
                              if($value['order_status']==8){ echo '<b style="color:green">Return Completed</b>';
                            }?>
                              
                            </td>

                            <td><?php echo date("d-m-Y h:i A",$value['add_date']);?> </td>
