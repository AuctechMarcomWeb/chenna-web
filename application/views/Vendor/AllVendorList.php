                            ...
                            foreach ($vendors as $v): ?>
                                <tr>
                                    ...
                                    <td>
                                        <label class="switch">
                                            <input type="checkbox" <?= $v->status == 1 ? 'checked' : ''; ?>
                                                value="<?= $v->id; ?>" data-role="vendor"
                                                onclick="toggle_vendor_status(this)">
                                    ...
                                    <td>
                                        ...
                                        <!-- View Button -->
                                        <a href="<?= site_url('admin/vendor/Vendor_view_details/' . $v->id); ?>"
                                            class="btn btn-sm btn-info">
                                            View
                                        </a>

                                        <!-- Edit Button -->
                                        <a href="<?= site_url('admin/vendor/edit/' . $v->id); ?>"
                                            class="btn btn-sm btn-warning">
                                            Edit
                                        </a>

                                        <!-- Delete Button -->
                                        <button class="btn btn-sm btn-danger" onclick="deleteVendor(<?= $v->id; ?>)">
                                            Delete
                                        </button>
                                    </td>
...
<script>
    function toggle_vendor_status(el) {
        ...
        let successMsg = status == 1
            ? 'Vendor has been approved successfully. Login details sent by email.'
            : 'Vendor has been set to pending (unapproved).';
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: '<?= site_url("admin/vendor/admin_appro_vendor_users") ?>',
                    type: 'POST',
