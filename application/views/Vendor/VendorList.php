                ...
                foreach ($getData as $value) {
                  ...
                  /*Seller Profile Completion Percentage*/
                       ...
                       $per = '10';  
                            ...
                            if($staff['mobile_verify']=='1'){ $per  = $per+'15';} else {$per = $per;}
                            if($staff['email_verify']=='1'){  $per  = $per+'15';} else {$per = $per;}
                            if(!empty($staff['profile_pic'])){ $per = $per+'5'; } else  {$per = $per;}

                            $doc = $this->db->get_where('staff_kyc_document',array('staff_id'=>$value['id']))->num_rows();

                            if($doc=='0'){ $per = $per; } else if($doc=='1') { $per =$per+'10'; } else if($doc=='2') {$per = $per+'20';} else if($doc=='3') { $per = $per+'30'; } else if($doc=='4') { $per =$per+'40';}
