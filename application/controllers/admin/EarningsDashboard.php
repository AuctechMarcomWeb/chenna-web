...
class EarningsDashboard extends CI_Controller
{
    ...
    public function getSummaryAjax()
    {
        ...
        $adminData = $this->session->userdata('adminData');
        $filters['user_type'] = $adminData['Type'];
        $filters['user_id'] = $adminData['id'];

        $summary = $this->EarningsDashboard_model->getSummary($filters);
    ...
    public function getVendorsByPromoter()
    {
        ...
        $vendors = $this->db->where('promoter_id', $promoter_id)->get('vendors')->result();
