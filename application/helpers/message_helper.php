...
function getAlertMessage($code){
    $codes = Array(
                '1' => 'Invalid Email ID or Password.',
                ...
                '9' => 'Invalid Mobile Number or Password.',
                //Image Upload Alerts
                ...
                '154' => 'Mebmership Added successfully.',
                '155' => 'Error!!!!!!!!!!!!!!!.',
                '156' => 'successfully.',
...
function is_logged_in_website() {
    $obj =& get_instance();
    $UserData = $obj->session->userdata('Userlogindata');
    if (!empty($UserData)) {
        redirect('Product/myAccount');
    }
}
// Check login status if not login redirect it to login page
function is_not_logged_in_website() {
    $obj = & get_instance();
    $UserData = $obj->session->userdata('Userlogindata');
   // print_r($adminData); exit;
    if (empty($UserData)) {
        redirect('Product/ekart');
    }
}
...
function sentEmailInfo($email,$smsmessage,$subject=''){
            // ++++++++++++++
          if(empty($subject)){
             $subject = "Welcome to ".PROJECT_tit."\r\n";
          }
          $to      = $email;
          $subject = $subject;
          $message ='';
  ...
  function SendOtpSMS($requestMobile,$message){
    //   $url = "http://bulksms.megainfomatix.com/http-api.php?username=lalitmohan&password=123456&senderid=MRNMRS&route=2&number=".$requestMobile."&message=".$message.""; 
    //         $ch = curl_init();
    ...
    //         curl_exec($ch); 
      ...
      function AlertMSG($type,$data){
            if ($type=='S' || $type=='s' || $type=='success' || $type=='Success') {
                 $message = '<div class="alert alert-success er_msg alert-dismissible"><button type="button" class="close" data-dismiss="alert">&times;</button>'.trim(ucfirst($data)).'</div>';
            }else{
                 $message = '<div class="alert alert-danger er_msg alert-dismissible"><button type="button" class="close" data-dismiss="alert">&times;</button>'.trim(ucfirst($data)).'</div>';
            }
            return $message;
      }