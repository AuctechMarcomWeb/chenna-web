 ...
 */    
   ...
   function singlerowparameter2A($select,$matchWith,$matchingId,$table){
        $tableRecord = & get_instance();
        $tableRecord->load->database();  
        $tableRecord->db->select($select);  
        $query = $tableRecord->db->get_where($table,array($matchWith => $matchingId))->row_array();
        if(empty($query['product_name'])){
            $tableRecord->db->select($select);  
            $query = $tableRecord->db->get_where($table,array($matchWith => $query['reference']))->row_array();
        }
        return $query['product_name']; 
   }


function saveProfilesImageBoy($base64,$folder){
                ...
                //$path      = $_SERVER['DOCUMENT_ROOT'];
                 $path = BANNER_DIRECTORY.'/'.$folder.'/'.$imageName;
   ...
   function allData($table,$matchWith,$matchingId)
   {
       ...
       function saveProfilesImage1($abc){
                   
             //    $img = imagecreatefromstring(base64_decode($abc)); 
                  $img       = imagecreatefromstring(base64_decode($abc));
                ...
                { 
                    $imageName = time().'.jpg';
                    $path = $_SERVER['DOCUMENT_ROOT'];  
                   //$path = $path.'/assets/profile_image/'.$imageName;
...
function generateServerResponse($msg_code, $res_msg, $data='',$dynamicValue=''){
        ...
        $array['ecommerce']["res_code"] = $msg_code;
        $array['ecommerce']["res_msg"]  = $resultMsg;
        $array['ecommerce']["sync_time"]= $getDateTime['date'];
         
             
        if(!empty($data)){
            foreach($data as $key=>$val){
                $array['ecommerce'][$key]  = $val;
            }
        }
    ...
    function validateJson($requestJson, $check_request_keys){
        if($requestJson){
            ...
            // foreach($requestJson['shopmet'] as $key=>$val){
            foreach($requestJson as $key=>$val){
                $validate_keys[] = $key;
            }
            
            // echo "<pre>";print_r($validate_keys);
            // echo "<pre>";print_r($check_request_keys);
    ...
    function Messages($res_msg='',$dynamicValue=''){
        $codes = Array(
                        ...
                        '208' => 'School & Branch',
                        '209' => 'Payment Failed ',      
                        '210' => 'Login successfully done.', 
                        '211' => 'Logout successfully done.',
                        '212' => 'Product Deleted successfully.', 
                        '213' => 'Signature Completed successfully.', 
                        '214' => 'Product Submit successfully.',
                        '215' => 'delivery not possible this address.',                          
                        '216' => 'Grocery products delivery not possible outside the Bijnor City.',                          
                        '217' => 'Mebmership Added successfully.',                     
                        '218' => 'Error!!!!!!!!!!!!!!!!!.',     
                        '219' => 'Old password is not correct.', 
                        '220' => 'Password change successfully.',                 
                        '221' => 'Confirm password have been successfully changed.', 
                        '222' => 'Invalid Mobile or password.',
                        '223' => 'Email-id is already exist.',                 
                        '224' => 'Registeration have been successfully.',
                        '225' => 'your password have been send on your registered  Mobile number.',   
                        '226' => 'Facebook id can not empty.',                 
                        '227' => 'Sorry! your account have been deactivetd contact to admin.', 
                        '228' => 'Country Id can not empty.',   
                        '229' => 'your payment have been failed thats why order is canceled.',   
                        '230' => 'Order successfully done.',   
                        '231' => 'Category Id can not empty.',  
                        '232' => 'Product Added To WishList.',   
                        '233' => 'Product Remove From WishList.',   
                        '234' => 'Product Already Added in WishList.',
                        '235' => 'Order Cancelled Successfully.',   
                        '236' => 'Product Cancelled Successfully.', 
                        '237' => 'Product Return request Successfully Done.',   
                        '238' => 'Order Return request Successfully Done.',  
                        '239' => 'OTP Verify Successfully.',
                        '240' => 'Old password and confirm password does not matched.', 
                        'E' =>   'Data Not Found',
                        ...
                        '241' => 'Mobile is not registered.',
                        '242' => 'Mobile is not exists.',
                        '243' => 'To cancel the Order please contact our Customer support.', 
...
//********************************************  Top10 SMS Panel code Start  *********************************

  


function sendSMS2($mobile,$message,$template) {

//Route=11 for Exact template and correct template ID    (For SHPMET )
//Route=2 for Mismatch template and correct template ID   (For Default )

 // $url = "http://sms.teleosms.com/api/mt/SendSMS?user=abhi241092&password=12345&senderid=SHPMET&channel=Trans&DCS=0&flashsms=0&number=".$mobile."&text=".urlencode($message)."&route=11&peid=1301160023932729644&dlttemplateid=".$template."";
...
//  // $url = "http://sms.teleosms.com/api/mt/SendSMS?user=abhi241092&password=12345&senderid=SHPMET&channel=Trans&DCS=0&flashsms=0&number=".$mobile."&text=".urlencode($message)."&route=2&peid=1301160023932729644&dlttemplateid=".$template."";


//  $url = "http://roundsms.com/api/sendhttp.php?authkey=MmFhYWEwOTQ1ZmF&mobiles=".$mobile."&message=".urlencode($message)."&sender=SHPMET&type=1&route=2";
//       $ch = curl_init();
//         curl_setopt($ch, CURLOPT_URL,$url);
//         curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
//          curl_exec($ch); 
...
function sendSMS_old($mobile,$message,$template) {
...
function sendSMS($mobile, $message, $template) {
    $apiKey = "333464756b656b6172745f757365723130301718705089"; // Place your API key here

   // $url = "http://sms.sibook.in/http-tokenkeyapi.php?authentic-key=" . $apiKey . "&senderid=DUKRLT&route=2&number=" . $mobile . "&message=" . urlencode($message) . "&templateid=" . $template;
 $url = "http://sms.azmobia.com/http-tokenkeyapi.php?authentic-key=" . $apiKey . "&senderid=DUKRLT&route=2&number=" . $mobile . "&message=" . urlencode($message) . "&templateid=" . $template;
...
function sentCommonEmailtest($email,$smsmessage,$sub){
    $message='';
        // ++++++++++++++
     // $to      = $email;
     $to      = "dukekart110@gmail.com";
