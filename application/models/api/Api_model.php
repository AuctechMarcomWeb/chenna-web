...
/*User Registration*/
    ...
    public function send_otp_new($request) {
        $field['mobile'] = $request['mobile'];
        $check = $this->db->get_where('user_master',array('mobile' => $request['mobile']))->row_array();
        ...
        if(empty($check))
        {
                ...
                $this->db->insert('user_master',['is_registered'=>'1','mobile'=>$request['mobile'],'otp'=>$otp,'add_date'=>time()]);
    ...
    public function verifyOtp_new($request) {
        $field['mobile'] = $request['mobile'];
        $check = $this->db->get_where('user_master',array('mobile'=>$request['mobile']))->row_array();
...
public function userSignUp($request) { 
    ...
    if(!empty($check)) {
        ...
        // $message_content='Your customer account has been successfully registered with Dukekart Go and check out soon https://www.Dukekart.com/ for exciting offers and discount';
    } else {
    ...
    //     $email_content='Your customer account has been successfully registered with Dukekart. Welcome to a new world of online shopping. Go and check out soon https://www.Dukekart.com/ for exciting offers and discount. You will get best shopping experience and great services with us';

    //     //******************** Email temp ****************************************

    //     $status="Registration Successful";
    //     $user= $request['name'];
    //     $subject="Your customer account registration with Dukekart has been successful";
    //     $dashboard='https://dukekart.in/';
...
public function dashbordList1($request) {
    ...
    $sliderList =$this->db->get_where('banner_master',array('status'=>'1'))->result_array();
     ...
     $this->db->select('id,banner_image');
    $offerList =$this->db->get_where('banner_master',array('level'=>'2'))->result_array();
    ...
    $main_categories = array (
                          ...
                          array("Id"=>"7","Name"=>"Chicken Meat","Thumb"=>"https://dukekart.in/My-Img/Exotic.png"),
                           array("Id"=>"13","Name"=>"Dairy Items","Thumb"=>"https://dukekart.in/My-Img/milk.jpeg"),
                          array("Id"=>"14","Name"=>"Grocery","Thumb"=>"https://dukekart.in/My-Img/Herbs.png"),
                          array("Id"=>"16","Name"=>"Spices","Thumb"=>"https://dukekart.in/My-Img/Spices.jpeg")
                    );
      ...
      /*Slider Array Here*/
       ...
       $slider   = array();
        ...
        $main_array['Slider'] = $hold_slider;
         ...
         /*Offer Slider Array Here*/

       $Offerslider   = array();
         
         foreach ($offerList as $key => $offerList) {
           $Offerslider['Id']     = $offerList['id'];
           $Offerslider['image']  = base_url().'/assets/banner_images/'.$offerList['banner_image'];
           $hold_slider_offer[]    = $Offerslider;
         }

        $main_array['OfferSlider'] = $hold_slider_offer;  
...
//***********************************************************************************************************
     ...
     $response['image1']                       =  base_url().'/assets/product_images/'.$Product['image1'];
...
public function AddToCardNew($request) {
    ...
    //    $userPincode = $this->db->get_where('user_address_master',array('user_master_id'=>$request['user_id']))->row_array();
        
        
     //    $checkPincode = $this->db->get_where('pin_code_master',array('pin_code'=>$userPincode['pincode']))->row_array();
       ...
       foreach ($cartList as $key => $value) {
        $Product = $this->db->get_where('sub_product_master',array('id'=>$value['product_master_id']))->row_array();
         ...
         $fields['color']              = $Product['color']*$value['quantity'];
...
//***********************************************************************************************************
      ...
      public function checkPincode($request) {
      ...
      //  $userPincode = $this->db->get_where('user_address_master',array('user_master_id'=>$request['user_id']))->row_array();
        ...
        $checkPincode = $this->db->get_where('pin_code_master',array('pin_code'=>$request['pincode']))->row_array();
...
public function placeOnlineOrderCopy($request) {
...
//              $this->db->select('*');
// $this->db->where(array(
//     'address_master_id' => $request['address_id'],
//     'user_master_id' => $request['userId']
// ));
// $this->db->order_by('add_date', 'DESC');
// $this->db->limit(1);
            $this->db->select('final_price,user_master_id');
            if($request['paymentType']==='1'){
              //  $getPrice = $this->db->get('order_master')->row_array();
                $getPrice = $this->db->get_where('order_master',array('order_number'=>$isProductBuys))->row_array();  
            }else{
               // $getPrice = $this->db->get('order_master2')->row_array();
            ...
            // $admin_message = "Dukekart has received an order on Product name:".$product['product_name']." Seller name:".$staff['name']." Mobile no:".$staff['mobile']."";
                  // $admin_message = "Dukekart has received an order on Product name:".$product['product_name']." Seller name:".$staff['name']." Mobile no:".$staff['mobile']."";
                    
            sendSMS($staff['mobile'],$seller_message,'1007123141312662937'); //pending
           // sendSMS('9935149155',$admin_message,'1307161760182866974'); //pending
          //  sendSMS('7460833766',$admin_message,'1307161760182866974'); //pending
        } else {
...
/*Buy Pruduct Place Order use this Function*/
    ...
    public function buyProduct($data) {
        ...
        if(count($cart) > 0) { 
            ...
            $fields['address_master_id']       = $data['address_id'];
            ...
            foreach ($cart as $value) {
                ...
                if($this->db->affected_rows() > 0) {
                  ...
                  $shop = $this->db->get_where('shop_master',array('id'=>$productData['shop_id']))->row_array();
                   ...
                   // $admin_message = "Dukekart has received an order on Product name:".$productData['product_name']." Seller name:".$staff['name']." Mobile no:".$staff['mobile'].".  Regards, Dukekart Real Time Private Limited , www.dukekart.in";
                     $admin_message = "Dukekart has received an order on Product name: " . $productData['product_name'] . ", Seller Mobile no: ". $staff['mobile'] .".  Regards, Dukekart Real Time Private Limited , www.dukekart.in";
            /*Seller Place Order Message*/
                    /*Seller Place Order Message*/
                    $seller_login_link = "https://dukekart.in/seller-login";
                    $seller_message="You have received an order on ".$order['order_number']." of SKU ".$productData['sku_code'].". Check this link ".$seller_login_link." and confirm the order from your side. For help contact 8934990990 -From Dukekart";
                    if($data['paymentType']=='1'){
            ...
            $textPDF = $this->pdfGenrate($lastId,$link,$data['paymentType']);
    ...
    function pdfGenrate($pakage_master_id,$pdfFilePath,$type) {
        error_reporting(0);
        
         if($type=='1'){
                $OrderDetail = $this->db->order_by('id','user_master_id', 'desc')->get_where('order_master', array('id' => $pakage_master_id))->row_array();
                ...
                $OrderDetail = $this->db->order_by('id','user_master_id', 'desc')->get_where('order_master2', array('id' => $pakage_master_id))->row_array();
                $getProduct = $this->db->get_where('purchase_master2', array('order_master_id' => $pakage_master_id))->result_array();
                $getAddress = $this->db->get_where('order_address_master2', array('order_master_id' => $pakage_master_id))->row_array();
            }
...
$html.='<div class="container" style="margin: 0 70px;background-color: white;margin-top:20px;border:4px solid rgb(239,126,45);height: auto;padding: 20px;border-radius: 10px;font-size: 15px;">
    <img class="img-fluid" src="https://dukekart.in/assets/img/logo2.png" alt="Dukekart" style="position: absolute;top: 0;height: 42px;width: 200px;">
...
$OrderDetail['add_date']).')</span></td>
   ...
   <table style="width:100%;padding: 0px 10% 0 13%; margin: 5px 0px 10px;">
      ...
      <tr>
        <td><img src="https://dukekart.in/assets/img/logo2.png" alt="Dukekart" style="height: 38px;width: 100px;"></td>
...
function sentCommonEmail2($email,$smsmessage,$sub){
      ...
      $headers = "From:"."Dukekartcs@gmail.com"."\r\n";
    ...
    function pdfGenrate_old21032024($pakage_master_id,$pdfFilePath) {
        ...
        $html .= '<div style="float:left;width:100%; margin-top:20px;"><div style="float:left;width:70%"><div class="image"><h3>ASAM Dukekart PRIVATE LIMITED</h3></div><div style="margin-left: 30px; top: -60px" > 
      ...
      $html .= '<div style="float:left;width:70%;margin-left:40%"><table><tr><td><p style="font-size:18px;">Thank you...</p>Order By - </td><td><div class="image" style="margin-top:4px;"><br><h4>Dukekart</h4></div><div style="top: -60px">
        </div></td></tr></table></div>
      ';        
...
public function forgotPasswordOtp($request){
 ...
 if(!empty($check)){
  ...
  // $message ="Dear ".$check['username'].", Your Forgot password OTP is ".$otp." Thanks. ASAM Dukekart PRIVATE LIMITED.";
   
   //New
   
  // $message ="Otp Test";
  
  $message = $otp. " is your One Time Password (OTP) You can now reset your password with the given otp! - From www.Dukekart.com";
  // $message = $otp." is your One Time Password (OTP) You can now reset your password with the given otp! - From www.Dukekart.com";
